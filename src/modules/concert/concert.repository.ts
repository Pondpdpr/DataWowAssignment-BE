...
export class ConcertRepository extends Repository<Concert> {
  ...
  async getStat(): Promise<StatDto> {
    const result = await this.createQueryBuilder('c')
      ...
      .select([
        'SUM(CASE WHEN c.deletedAt IS NULL THEN c.limit END) AS seats',
        'COUNT(CASE WHEN r.deletedAt IS NULL AND c.deletedAt IS NULL THEN r.id END) AS reserved',
        'COUNT(CASE WHEN r.deletedAt IS NOT NULL AND c.deletedAt IS NULL THEN r.id END) AS canceled',
      ])
      .getRawMany();
    return result[0];
  }
